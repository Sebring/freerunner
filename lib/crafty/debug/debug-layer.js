...
Crafty.c("DebugRectangle", {
    ...
    // freerunner - aabb support
    debugAABB(aabb) {
        this.debugAabb = aabb;
        this.unbind("DebugDraw", this.drawDebugAABB);
        this.bind("DebugDraw", this.drawDebugAABB);
        return this;
    ...
    drawDebugAABB(ctx) {
        ...
        let x = this.debugAabb.min.x
        let y = this.debugAabb.min.y
        let w = this.debugAabb.max.x - x
        let h = this.debugAabb.max.y - y 
        if (this._debug.fillStyle)
...
Crafty.c("MapGrid", {
    init: function () {
        this.requires("2D, DebugCanvas")
        this.bind("DebugDraw", this.drawMapGrid)
    },
    drawMapGrid(ctx) {
        ctx.strokeStyle = 'silver'
        ctx.font = '18px sans-serif';
        ctx.fillStyle = 'silver'
        let size = Crafty.s('HashMap')._cellSize
        let map = Crafty.s('HashMap').map
        if (!map || !map.length) return
        for (let x=0; x<map.length; x++) {
            if (!map[x] || !map[x].length) continue
            for (let y=0; y<map[x].length; y++) {
                if (!map[x][y]) continue
                let count = map[x][y].length
                if (count) {
                    ctx.strokeRect(x*size, y*size, size, size)
                    ctx.fillText(count, x * size + (size * 0.2), y * size+(size*0.8))
                }
            }
        }
    }
...
Crafty.c("WiredHitBox", {
    ...
    matchHitBox: function () {
        this.debugPolygon(this.hitmap.translated || this.hitmap.orig);
    }
});
